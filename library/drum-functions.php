...
// Add post/page name and animsition to body class
...
function my_theme_register_required_plugins() {
    $plugins = array(
        ...
        // Require ACF Address Field
        array(
            'name'      => 'Advanced Custom Fields: Address',
            'slug'      => 'acf-field-address-master',
            'source'    => 'http://drumcreative.com/wp-content/required-plugins/acf-field-address-master.zip', // The plugin source.
            'required'  => true,
        ),
...
// Add a Technical Support link to admin toolbar
...
add_action('admin_bar_menu', 'support_toolbar_link', 999);
...
add_filter( 'acf_the_content', 'style_ul_filter' );
add_filter( 'the_content', 'style_ul_filter' );
...
// Add button drop-down to WYSIWYG
function drum_mce_buttons($buttons) {
	array_unshift($buttons, 'styleselect');
	return $buttons;
}
add_filter('mce_buttons', 'drum_mce_buttons');


/*
* Callback Function for MCE-Editor
*/
function drum_mce_before_init_insert_formats( $init_array ) {
...
add_filter( 'tiny_mce_before_init', 'drum_mce_before_init_insert_formats' );
?>